    ...
    methods: {
      submit() {
        ...
        if (this.$refs.myBrandForm.validate()) {
          ...
          //categories.map(c => c.id)---> [1,2,4]--join--->"1,2,4"
          ...
          // console.log(sthis.$qs.stringify(params));
          this.$http({
            method: this.isEdit ? 'put' : 'post',
            url: '/item/brand/addOrEditBrand',
        ...
        /*
        *
        * 1、如果前端传参数的方式是：js对象，后台的接收方式：用对象接收，加入@RequestBody
        * 2、如果前端name=123&image=&cids=3%2C80&letter=A，字符串的方式传参，后台要用对象接收，不需要加注解。直接可以用对象接收
        *
        * 如果后台用单独的字段去接收参数，需要加@RequestParam注解
        *
        * */
      },
      clear() {

    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="deleteBySpuId(props.item.id)">
            <i class="el-icon-delete"/>
          </v-btn>
          <v-btn icon v-if="props.item.saleable" @click="upOrDown('0',props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="upOrDown('1',props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      deleteBySpuId(id){
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          this.$http.get("/item/spu/deleteById/" + id)
            .then((res) => {
      ...
      upOrDown(s,id){
        ...
        this.$message.confirm('是否要'+name+'?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {

          this.$http.get("/item/spu/upOrDown/" , {
            params:{
              spuId :id,
              saleable :s
            }
